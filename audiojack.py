...
class AudioJack(object):
    ...
    def _get_metadata(self, parsed):
        ...
        mb_results = musicbrainzngs.search_recordings(query=parsed['title'], artist=artist, artistname=artistname,
                                                      limit=20)
        dmbresults = mb_results
        for i, recording in enumerate(mb_results['recording-list']):
            if 'release-list' in recording:
                ...
                if ('artists' not in parsed or re.sub(r'\W', '', title.lower()) == re.sub(r'\W', '', parsed[
                    ...
                    for j, release in enumerate(recording['release-list']):
                        if self._valid(release):
                            print(i, j)
                        album = release['title']
                        album_id = release['id']
                        date = release['date'] if 'date' in release else None
                        entry = {
                            'url': parsed['url'],
                            'title': title,
                            'artist': artist,
